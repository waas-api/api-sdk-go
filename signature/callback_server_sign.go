...
func CallbackServerGenResponseSign(resBody interface{}, priKey string) (newResBody interface{}, err error) {
	var resBase struct {
	...
	if bs, ok := resBody.([]byte); ok {
		...
		if bs, err := json.Marshal(resBase); err != nil {
			...
			newResBody = new(map[string]interface{})
			if err := json.Unmarshal(bs, &newResBody); err != nil {
				return resBody, err
			}
...
//	@param priKey rsa private key, generated by shop client.
//	@return sign new signature string
